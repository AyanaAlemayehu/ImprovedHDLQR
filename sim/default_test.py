...
#TRACKED SIGNALS (default to AXI Stream) (i added aresetn cause thats important for modeling)
SIGNALS = ['axis_tvalid','axis_tready','axis_tlast','axis_tdata','axis_tstrb', 'axis_aresetn']
    ...
    async def _monitor_recv(self):
        ...
        while True:
            ...
            if (self._checker):
                ...
                if valid and ready:
                    self.transactions += 1
                    thing = dict(data=data,last=last,name=self.name,count=self.transactions,reset=reset)
                    self._recv(thing)
                    if (self.history):
...
async def test_a(dut, *args):
    ...
    await set_ready(s00_axis_aclk, dut,1)
    await reset(dut.s00_axis_aclk, dut.s00_axis_aresetn,2,0)
    # feed the driver
    # either single(s)
    data  = {'type':'single', "contents":{"strb": , "last": , "data": }}
    #or burst
    data  = {'type':'burst', "contents":{"data": []}}

    #send the data
    tester.input_driver.append(data)
    
    # wait and apply backpressure
    await ClockCycles(dut.s00_axis_aclk, ) #figure out how many cycles to wait for a result
    raise Exception("Not Implemented (how many cycles you should wait for a result)")
...
def runner():
    ...
    sources = [proj_path / "hdl" / "verilog_file.sv"] #grow/modify this as needed.
